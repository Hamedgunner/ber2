...
print ("welcome, Created By Hamed barzkar\n")

print ("Please subscribe to the channel to receive updates! : for telegram : @tech_ir_media\n")
				...
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
							...
							try:
								bot.sendMessage(target, "ساخته شده توسط @del_bot", message_id=msg.get("message_id"))
							except:
								print("err admin")
							...
							try:
								bot.sendMessage(target, "جــون دلوم برار🥺", message_id=msg.get("message_id"))
							except:
							...
							try:
								bot.sendMessage(target, "عجب قیافه ای داری برار🙄", message_id=msg.get("message_id"))
							except:
